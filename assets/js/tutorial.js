...
(function () {
  ...
  var SPECIMEN_STEPS = [
    {
      selector: null,
      title: '歡迎使用',
      body: '本介面源自<strong>血液組</strong>與<strong>長庚醫院人工智慧實驗室</strong>合作開發血球辨識 AI 的實際需求，透過使用者研究與問卷調查，以<strong>醫檢師的日常工作流程</strong>為核心進行介面設計，目標是讓 AI 能與臨床需求真正接軌並減輕工作負擔，此介面設計假定 AI 能力與真人相當。<br/>本教學將帶您依序走過：<strong>檢體管理</strong> → <strong>影像檢視與細胞編輯</strong> → <strong>報告核發</strong> 的完整流程，協助您快速上手並理解每一個設計背後的用意。'
    },
    {
    ...
    },
    {
      ...
      title: '左上：篩選與模式',
      body: '<strong>數位閱片 / 實體作業</strong>：用來切換工作模式。<strong>數位閱片</strong>集中顯示需在電腦螢幕上判讀的檢體；<strong>實體作業</strong>則顯示需要推片、查看實體玻片或採血管的檢體，讓發報告醫檢師與鏡檢醫檢師可以分流，各自只看到自己需要處理的清單。<br/><strong>篩選</strong>：展開後可勾選要顯示的檢體狀態（如 PLT Check、Digital Review 等），醫檢師可以額外自訂想看到的檢體類別。<br/><strong>檢驗日期、送檢單位、機台、檢體搜尋</strong>：可依日期區間、送檢單位、機台與檢體 ID／病歷號多條件組合篩選，在<strong>高檢體量</strong>情境下，先利用這些條件縮小範圍，有助於更快找到需要處理的檢體。'
    },
    {
  ...
  var IMAGE_STEPS = [
    ...
    },
    {
      ...
      title: '右下：細胞區塊與編輯方式',
      body: '細胞會依分類群組顯示（例如 Segmented、Lymphocyte、Blast 等），達留單標準的異常細胞群組會置頂並以紅色強調，確保高風險細胞不會被忽略。群組標題同時顯示該類別的細胞數與百分比，當數值達到實驗室設定的留單閾值時，標題也會變色提醒您注意。<br/><strong>選取方式</strong>：<br/>單顆選取：左鍵點擊。<br/>多選：按住 Ctrl + 左鍵點擊多顆。<br/>範圍選：在同一群組內按住 Shift + 左鍵選取區間。<br/>單手模式：按住滑鼠右鍵不放，同時以左鍵點擊多顆，放開右鍵後會在游標旁彈出分類選單；此模式設計用意是<strong>減少左手頻繁前伸操作鍵盤或調整姿勢</strong>，降低長時間操作造成的肢體疲勞。<br/><strong>編輯方式</strong>：您可以將選取的細胞拖曳到其他類別區塊，或是按右鍵開啟分類選單。右鍵選單會優先顯示 AI 建議的分類與信心度，並將常用、異常與其他類別分層排列且放大按鈕，讓您在不需非常精準控制滑鼠的情況下，也能快速、準確地完成重分類。'
    },
    {
  ...
  var REPORT_STEPS = [
    ...
    },
    {
      ...
      title: '左上：標題與風險橫幅',
      body: '<strong>檢體編號與狀態膠囊</strong>：顯示目前檢體編號與其狀態標記，方便您確認正在簽核的是哪一筆檢體。<br/><strong>風險橫幅</strong>：僅顯示兩種狀態——<strong>紅色</strong>代表已達留單標準，簽核前請再次確認是否需人工鏡檢或留單；<strong>綠色</strong>代表未達留單標準。此橫幅的設計目的，是在<strong>簽核前最後一刻主動提醒風險</strong>，降低誤發需留單報告的機率。'
    },
    {
